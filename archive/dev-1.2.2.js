(() => {
  ...
  var kebabCase = (string) => string.replace(/[A-Z]/g, "-$&").toLowerCase();
  var compose = (objects) => {
  ...
  define.register(5, "template", (context, [args]) => {
    if (!args) {
      const constructor2 = function(meta) {
        meta.root = this;
      };
  ...
  define.register(7, Symbol(), (context) => {
    ...
    const constructor = function(meta) {
      ...
      meta.x.elements = new Proxy((selector) => meta.root.querySelector(selector), {
        get: (find, name) => cache[name] ??= find(`#${kebabCase(name)},#${name}`) ?? find(kebabCase(name))
      });
    };
    return { constructor };
  });
