...
Similar warnings after this one will be suppressed`);
  ...
  var Flow = class {
    ...
    then(callback) {
      ...
      return this.pipe((next, ...args) => {
        if (!this.#stopping && isAwaited)
          warn`flow-awaited-without-once`;
        callback(...args);
        next();
      });
    }
  ...
  var LiveCore = class _LiveCore {
    ...
    __alter(alteration, deepChanges = /* @__PURE__ */ new Set()) {
      ...
      const keys = this.__value instanceof Object ? Object.keys(this.__value ?? {}) : [];
  ...
  monitor.register = (name, registration) => {
    if (name == "result")
      warn`monitor-should-not-register-result`;
    if (registrations[name])
  ...
  define.register(1, "meta", (context2, argslist) => {
    ...
    if (uniqueProperties.size != properties.length) {
      ...
      if (name)
        error`define-property-${name}-conflicing`;
    }
